...
Így a választás a Dante protokollra esett, ezért a szakdolgozatomban 
megtervezésre kerülő hangrendszer lelke ez a protokoll lesz.
Hangládák szempontjából a Martin Audio Wavefront Precision szériás 
termékein belül kétfajta rendszerre is esett választás.
Ezekről a későbbiekben részletesen lesz szó.
Mivel teljes mértékben digitális rendszer elérése volt a cél, ezért a Dante modullal
...
A szoftvermérnökkel való e-mail-es beszélgetésünk során megköszönte a probléma
...
\begin{figure}[H]
	...
	\caption{E-mail a Martin Audio szoftvermérnökétől}
...
négy pólusú Neutrik Speakon csatlakozókon keresztül.
...
A hozzáadás gombra kattintva elénk kerül a fő kezelőfelület, ahol a hangrendszert tudjuk lépésről lépésre tervezni.
%----------------------------------------------------------------------------
\begin{figure}[H]
	\centering
...
Teljesen jobbra állítva kapunk maximális kompenzációt, de a rendszernek legalább 17dB headroom-ra van szüksége, hogy egyenes response-t kapjunk.
...
azon a napon 101800 Pa volt. Ezek beállítása után mivel az adott hangládához a gyári levegő veszteség kompenzációs beállítások nagyon jók, ezért nem változtattam rajtuk,
...
A \textit{``Target''} mezőben megadott érték a közönség területének súlyozása,
...
Ezek után a rendszert az említett paraméterek alapján összeépítjük, figyelve az összes program által megadott információra.
%----------------------------------------------------------------------------
\begin{figure}[H]
	\centering
...
A hangsúlyt az eltérésekre helyezem, és azokat fogom részletezni.
...
Ez persze nem jár kompromisszumok nélkül, a hangvisszaadás egyenletesebb ha egyesével hajtjuk a ládákat, és ezt meg is tudjuk tenni,
mivel rendelkezésünkre áll 2 db iK81 végfok erre a célra. 
Végeredményben tökéletesen kielégítő hangvisszaadást fog produkálni mint kiegészítő egység.
%----------------------------------------------------------------------------
\begin{figure}[H]
	\centering
...
A \textit{``Discover Devices''} gombra kattintva a szoftver megkeresi az összes végfokot a hálózaton, és megjeleníti azokat a
cím szerint növekvő sorrendben. Szinkronizáció után szabadon vezérelhetővé válnak az eszközök.
Egy kiválasztott eszközre jobb egérgombbal kattintva megjelenik egy menü, ahol az \textit{``Open Preset Manager''} opcióra kattintva
...
Tehát fontos a precíz és átgondolt munkavégzés. A hibák elkerülése után is hasznos számunkra a rendszer áttekintő diagram,
...
fognak jelet kapni. A következő lépésben a \textit{``Routing''} fülön be kell állítanunk, hogy az input csatorna Dante hálózatról fogja kapni a jelet.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=350px, keepaspectratio]{figures/dante_audioserver_patch.jpg}
...
Minden egyes eszköznél a \textit{``Lantecy''} oldalon láthatjuk az aktuális és átlagos késleltetési értékeket.
%----------------------------------------------------------------------------
\begin{figure}[H]
	\centering
...
a \textit{``Network Status''} fülre, ahol az összes eszköz késleltetési értékeit egyszerre láthatjuk. Ezen kívül még elérhető
metrikák:
%----------------------------------------------------------------------------
\begin{itemize}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=150px, keepaspectratio]{figures/sub_array_63hz.jpg}
...
Egy EndFire konfigurációs mélyláda elrendezést terveztem. 
...
a hangsebesség, ezáltal még tovább optimalizálható a rendszer, mivel nem egy előre átlagos fix értéket
...
%----------------------------------------------------------------------------
A jelenlegi rendszer két keverőpultot fog tartalmazni, egyet a fő hangrendszerhez, és egyet a monitorrendszerhez.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=60mm, keepaspectratio]{figures/sq6.jpg}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=45mm, keepaspectratio]{figures/sq_dante.jpg}
        \caption{A\&H SQ Dante kártya}\label{fig:sq_dante}
    \end{minipage}\hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=45mm, keepaspectratio]{figures/dt168.jpg}
...
%----------------------------------------------------------------------------
Ez a kiegészítő szerver egység lehetővé teszi bármilyen alacsony késleltetéssel dolgozó VST3 plugin használatát a rendszerben élő környezetben.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=50mm, keepaspectratio]{figures/rednet_pcie.jpg}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=300px, keepaspectratio]{figures/dt_preamp_control.jpg}
...
VST host szoftver. Képes több VST plugin egyidejű futtatására, és a pluginokat a felhasználó igényei szerint
rendszerezni, csoportosítani. Kifejezetten live használatra lett tervezve a kezdetektől fogva, ezáltal sok olyan
funkciót tartalmaz, mi egy FOH mérnök számára könnyebbé teszi a használatot. Például a pluginokat láncokba tudjuk
...
Így mind a 128x128 csatorna máris rendelkezésünkre áll a használatra.
...
(Elsőnek narancssárga figyelmeztetést kapunk, ha közel járunk a buffer méret határához.)
...
hangnyomása túl magas lesz, a szélső területeken pedig túl alacsony. Tehát a feladat adott, hogy a rendszer a lehetőségekhez mérten
...
a frontfill rendszert. A delay finder segítségével a két rendszer közötti késleltetési értéket
...
\begin{figure}[H]
	\centering
	\includegraphics[width=250px, keepaspectratio]{figures/nearfill_smaart.jpg}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=80mm, keepaspectratio]{figures/smaart_spl_meter.jpg}
